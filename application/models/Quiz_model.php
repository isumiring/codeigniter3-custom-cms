...
class Quiz_model extends CI_Model
{
     ...
     * @return array data
     */
    function GetAllQuizData($param=array()) {
        ...
        $data = $this->db
                ->select("quiz.*,COALESCE({$this->db->dbprefix('quiz_customer_answer')}.count_responder,0) as count_responder,quiz.id_quiz as id",FALSE)
                ->join(
                    "
                        (SELECT count(*) as count_responder,id_quiz FROM {$this->db->dbprefix('quiz_customer_answer')} GROUP BY id_quiz) 
                        {$this->db->dbprefix('quiz_customer_answer')}
                    ",
                    'quiz_customer_answer.id_quiz=quiz.id_quiz','left')
                ->get('quiz')
                ->result_array();
        return $data;
    }
     ...
     */
    function CountAllQuiz($param=array()) {
